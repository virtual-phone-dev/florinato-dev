...
export function ouvrirDB() {
  return new Promise((resolve, reject) => {
    const requete = indexedDB.open("MessagesDB", 14);
    const tables = ["annonces", "conversations", "comptes", "followers", "messages", "videos", "visites"];
...
export function useScrollIndexedDB({ nomStockage, donnees=[], lot=20, visible=true, onlineUsers=null,
	idConversation, idDestinataire, idCompte, idCompte2, idCompteConnecter, idPost, idProprietairePost, id, idAnnonce,
...
useEffect(() => {
  ...
  const nomProprietairePost = profil?.nameAccount ?? "";	
	...
	return (<>
		{afficherVideo && (<>
		<div className="video-card pb-30px" onClick={() => { setIdProprietairePost(api.idAccountChef); setIdCompte(api.idAccountChef); setIdDestinataire(api.idAccountChef); AvantMessagePage(api.idAccountChef); }}>
...
export function LesVideos({ data=[], setIdPost, setUrlVideo, setIdProprietairePost, setIdCompte, setIdDestinataire, clicVideo, nomcss, datecss,
	voirVideo, voirProfil, AvantMessagePage, dateParser, titrecss, cliccss, profilMap, video, affichagecss="video-grille", scrollX, nomEtphoto }) {
	...
	<div className={affichagecss} onScroll={scrollX || undefined}>
		{data.map((api) => (
			...
			<ChildApi66LesVideos 
				...
				setIdProprietairePost={setIdProprietairePost} setIdCompte={setIdCompte} setIdDestinataire={setIdDestinataire} AvantMessagePage={AvantMessagePage}
...
export function VideoSearchBlock({ data=[], profilMap, listVideo=[], valeur, setValeur, nomEtphoto, dataOverflow, overflow, AvantMessagePage,
  ...
  return (<>
	<RechercheTemplate 
		...
		setUrlVideo={setUrlVideo} profilMap={profilMap} AvantMessagePage={AvantMessagePage} />

	{overflow && (<>	
    <LesVideos 
		data={dataOverflow} setIdPost={setIdPost} setUrlVideo={setUrlVideo} 
		setIdProprietairePost={setIdProprietairePost} setIdCompte={setIdCompte} setIdDestinataire={setIdDestinataire} AvantMessagePage={AvantMessagePage} video 
	...
	return (
		<div className="page-blanche" onScroll={gererScroll}> 
			...
			<VideoSearchBlock 
				...
				clicVideo={clicVideo} voirVideo={voirVideo} voirProfil={voirProfil} video={video} AvantMessagePage={AvantMessagePage} nomEtphoto />
		</div>
)}
...
export function ModifierTemplate({ visible, fermer, valeur, setValeur, Valider, isLoading, infos, data, setIdCommentaire, setIdProprietaireCommentaire, profilMap,
	changerUrl, changerMiniature, titre, texte = "Écrivez l'url ...", textbtn="Valider", textarea, transVoirMiniature, miniature, setFileVideo, second, setSecond, 
	lesCommentaires, RepondrePage }) {
	...
	return (<>
        <div className="messageFA" onScroll={gererScrollMessages}>
          <div className="close">
            ...
            <div className="block-deux">
				<div className="b" onClick={PartagerContactPageFA}> <SvgFile/> </div>
        ...
        <div className="profilFA" onScroll={(e) => { gererScroll(e); gererScrollVisites(e); }}>
          <div className="head">
            <div className="close">
				...
				{messagebtn && (
				<div className="flex-no" onClick={OuvrirMessagePage}> <SvgMessage/><p>Message</p> </div>)}
            </div>
            {/* close */} 
          </div>
          {/* head */}
...
export function ListeDuMenu({ GestionDuCompte, MettreEnAvantCompte, ComptesRecentsPageFA, AdminFlorinato, Gestionnaire, MonArgentFA, FlorinatoPay, FlorinatoBusiness }) {
	return (<>
		<div className="list">
            ...
            <div className="b" onClick={FlorinatoPay}> <p>Florinato Pay</p> </div>
            <div className="b" onClick={FlorinatoBusiness}> <p>Monetisation</p> </div>
        </div>
        {/* list */}
  </>
)}
...
export function PopupDuBasTemplate({ visible, fermer, list, search, photo, titre, listAccount, valeur, setValeur, OuvrirMessagePage, setIdContact,
	Gestionnaire, GestionDuCompte, MettreEnAvantCompte, AdminFlorinato, ComptesRecentsPageFA, MonArgentFA, FlorinatoBusiness, FlorinatoPay }) {	
	if (!visible) return null;
          ...
          <div className="body">
            <div className="card">
				...
				<ListeDuMenu 
					...
					ComptesRecentsPageFA={ComptesRecentsPageFA} MonArgentFA={MonArgentFA} FlorinatoBusiness={FlorinatoBusiness} FlorinatoPay={FlorinatoPay} />)}
